...
data {
    ...
    real data_counts[Nsamples,Nbins-1];
...
model {
    ...
    for (m in 1:Nsamples) {
        ...
        probNorm = 0;
        for (n in 1:(Nbins-1)) {
            normprobs[n] = normal_lcdf(logradii[m,n+1] | mu, sigma) - normal_lcdf(logradii[m, n] | mu, sigma);
            probNorm += exp(normprobs[n]);
        }

        for (n in 1:(Nbins-1)) {
            normprobs[n] = normprobs[n] - log(probNorm);
        }


        P = diag_matrix(normprobs);
        M = P - normprobs*normprobs';

        sig_mvn = M * newCount;
        mu_mvn = normprobs * newCount;

        sig_mvn_inv = inverse(sig_mvn * sig_mvn')*sig_mvn';
        sig_mvn_inv = 0.5*(sig_mvn_inv + sig_mvn_inv');

        for (k in 1:(Nbins-1)) {
            mu_mvn[k] = mu_mvn[k] * newCount;
        }


        //print("DEBUG: mu_mvn ", mu_mvn);
        //print("DEBUG: sig_mvn ", sig_mvn);
        //print("DEBUG: sig_mvn_inv ", sig_mvn_inv);


        //print(mu_mvn);
        //data_counts_norm[m] ~ multi_normal(mu_mvn, sig_mvn);
        //data_counts_norm[m] ~ multi_normal_prec(mu_mvn, sig_mvn_inv);
        target += my_mvn_lpdf(data_counts_norm[m]' | mu_mvn, sig_mvn);
    }
}